  ...
  methods: {
    ...
    createAsciiTable(invoices) {
      ...
      for (const invoice of invoices.invoices) {
        for (const product of invoice.products) {
          table += `| ${invoice.id.toString().padEnd(lstr_id)} | ${invoice.date.padEnd(lstr_date)} | ${product.name.padEnd(lstr_name)} | ${product.price.toFixed(2).padEnd(lstr_price)} | ${product.quantity.toString().padEnd(lstr_qty)} | ${invoice.total_cost.toFixed(2).padEnd(lstr_tcost)} |\n`;
        }
        table +=`+${'-'.repeat(c_id.length)}+${'-'.repeat(c_date.length)}+${'-'.repeat(c_name.length)}+${'-'.repeat(c_price.length)}+${'-'.repeat(c_qty.length)}+${'-'.repeat(c_tcost.length)}+\n`;
      }
