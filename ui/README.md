...
## Livrable 1
Faites vos modifications aux fichiers `Home.vue`, `Navigation.vue`, `Product.vue` et `User.vue`. Ajoutez votre HTML dans la balise `<template>` et votre CSS dans la balise `<style>` de chacun des components. Vous pouvez également modifier la structure de fichiers si vous désirez.
